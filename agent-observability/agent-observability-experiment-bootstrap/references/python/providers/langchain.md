...
|---|---|---|
| `ChatOpenAI`, `OpenAI` | OpenAI | `providers/openai.md` |
| `AzureChatOpenAI` (with `azure_endpoint=`) | Azure OpenAI | Emit `AZURE_OPENAI_API_KEY` and `AZURE_OPENAI_ENDPOINT` asserts, not the `OPENAI_API_KEY` assert from `providers/openai.md` |
